...
('lucas@example.com', 'senha123'),
('fernanda@example.com', 'senha123'),
('gustavo@example.com', 'senha123'),
('mariana@example.com', 'senha123'),
('felipe@example.com', 'senha123');

-- INNER JOIN: Seleciona registros que têm correspondência em ambas as tabelas
SELECT usuario.nome, reserva.data_reserva, prato.nome_prato
FROM usuario
INNER JOIN reserva ON usuario.id_usuario = reserva.id_usuario
INNER JOIN prato ON reserva.id_prato = prato.id_prato;

-- LEFT JOIN: Seleciona todos os registros da tabela à esquerda, e os registros correspondentes da tabela à direita
SELECT usuario.nome, reserva.data_reserva, prato.nome_prato
FROM usuario
LEFT JOIN reserva ON usuario.id_usuario = reserva.id_usuario
LEFT JOIN prato ON reserva.id_prato = prato.id_prato;

-- RIGHT JOIN: Seleciona todos os registros da tabela à direita, e os registros correspondentes da tabela à esquerda
SELECT usuario.nome, reserva.data_reserva, prato.nome_prato
FROM usuario
RIGHT JOIN reserva ON usuario.id_usuario = reserva.id_usuario
RIGHT JOIN prato ON reserva.id_prato = prato.id_prato;

-- CROSS JOIN: Retorna o produto cartesiano entre as tabelas
SELECT usuario.nome, prato.nome_prato
FROM usuario
CROSS JOIN prato;